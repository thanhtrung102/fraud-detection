...
class TestReduceMemory:
    ...
    def test_reduce_memory_int(self):
        ...
        pd.testing.assert_frame_equal(df_reduced.astype(df.dtypes), df, check_dtype=False)
    ...
    def test_reduce_memory_float(self):
        ...
        np.testing.assert_array_almost_equal(
            df_reduced["small_float"].values, df["small_float"].values, decimal=5
        )

    def test_reduce_memory_with_missing(self):
        """Test handling of missing values."""
        df = pd.DataFrame({"col_with_nan": [1.0, np.nan, 3.0, np.nan, 5.0]})
